...
internal class OfflineCoordinator(private val root: File) {
  ...
  init { root.mkdirs(); root.listFiles()?.filter { it.name.endsWith(".part") || !OfflinePolicy.validKey(it.name) }.orEmpty().forEach { it.delete() } }
  @Synchronized fun enqueue(s:String,id:String,title:String,artist:String):Entry {
    ...
    val e=Entry(UUID.randomUUID().toString(),s,id,title.take(256),artist.take(256),"queued");items[key]=e
  ...
  @Synchronized fun cancel(op:String){ items.entries.firstOrNull{it.value.operationId==op}?.let{(k,e)->futures.remove(k)?.cancel(true);File(root,"$k.${e.operationId}.part").delete();e.status="cancelled";e.errorCode="CANCELLED";e.updatedAt=System.currentTimeMillis()} }
  @Synchronized fun remove(s:String,id:String){ val k=OfflinePolicy.key(s,id);futures.remove(k)?.cancel(true);items.remove(k);File(root,k).delete() }
  ...
  fun resolve(s:String,id:String):Entry? { val k=OfflinePolicy.key(s,id);val e=items[k]?:return null;val f=File(root,k);if(e.status!="ready"||!f.isFile||f.length()!=e.downloadedBytes||sha(f)!=e.digest){remove(s,id);return null};return e }
  ...
  private fun mediaUrl(e:Entry):String?=when(e.source){"netease"->"https://music.163.com/song/media/outer/url?id=${e.trackId.removePrefix("netrack_")}.mp3";"kugou"->null;else->null}
...
@ReactModule(name=OfflineAudioModule.NAME) class OfflineAudioModule(private val app:ReactApplicationContext):ReactContextBaseJavaModule(app){
  ...
  @ReactMethod fun enqueueDownload(m:ReadableMap,p:Promise){val allowed=setOf("source","trackId","title","artist","album","durationMs");val it=m.keySetIterator();while(it.hasNextKey())if(!allowed.contains(it.nextKey())){p.resolve(code("INVALID_REQUEST"));return};p.resolve(entry(c().enqueue(m.getString("source")?:"",m.getString("trackId")?:"",m.getString("title")?:"未知歌曲",m.getString("artist")?:"未知艺人")))}
  @ReactMethod fun cancelDownload(op:String,p:Promise){c().cancel(op);p.resolve(snapshot())};@ReactMethod fun retryDownload(s:String,id:String,p:Promise){c().remove(s,id);p.resolve(snapshot())};@ReactMethod fun removeDownload(s:String,id:String,p:Promise){c().remove(s,id);p.resolve(snapshot())};@ReactMethod fun clearDownloads(p:Promise){c().clear();p.resolve(snapshot())};@ReactMethod fun invalidate(s:String,id:String,p:Promise){c().remove(s,id);p.resolve(snapshot())}
