...
class DeepSeekModule(context: ReactApplicationContext) : ReactContextBaseJavaModule(context) {
    ...
    @ReactMethod fun translate(request: ReadableMap, promise: Promise) = complete(promise) {
        requireKeys(request, setOf("operationId", "provider", "sourceTrackId", "lyric", "title", "artist", "style", "lyricHash", "trackHash", "target", "consent", "allowNetwork", "forceRefresh"))
        ...
        result(client.translate(text(request, "operationId", 64), text(request, "provider", 16), text(request, "sourceTrackId", 128), text(request, "lyric", DeepSeekPolicy.MAX_LYRIC_BYTES), text(request, "title", DeepSeekPolicy.MAX_METADATA_CHARS), text(request, "artist", DeepSeekPolicy.MAX_METADATA_CHARS), text(request, "style", DeepSeekPolicy.MAX_STYLE_CHARS), text(request, "lyricHash", 64), text(request, "trackHash", 64), bool(request, "allowNetwork"), bool(request, "forceRefresh")).also { require(it.trackHash == null || it.trackHash == text(request, "trackHash", 64)) })
    }
