...
import com.listen2mobile.BuildConfig
...
class LocalAudioModule(private val app: ReactApplicationContext) : ReactContextBaseJavaModule(app), ActivityEventListener {
    companion object {
        ...
        private const val REPAIR_REQUEST_CODE = 7316
    ...
    private enum class PendingKind { IMPORT, LRC, REPAIR }
    private class Pending(
        val requestId: String,
        val promise: Promise,
        val recordId: String? = null,
        val kind: PendingKind = PendingKind.IMPORT,
    ...
    /** Issues a one-use app-private provider URI only for the current RNTP load. */
    @ReactMethod
    fun prepareLocalPlayback(recordId: String, playbackRequestId: String, promise: Promise) {
        if (!RECORD_ID.matches(recordId) || !REQUEST_ID.matches(playbackRequestId)) { promise.resolve(playback(recordId, playbackRequestId, "rejected", null, false)); return }
        worker.execute {
            val record = LibraryRepository.open(app).localRecord(recordId)
            if (record == null || record.availability != "available" || privateStore.document(recordId) == null) {
                promise.resolve(playback(recordId, playbackRequestId, "unavailable", null, false)); return@execute
            }
            val token = LocalPlaybackTokens.issue(recordId, playbackRequestId)
            if (token == null) { promise.resolve(playback(recordId, playbackRequestId, "rejected", null, false)); return@execute }
            promise.resolve(playback(recordId, playbackRequestId, "success", LocalMediaPolicy.providerUri("${BuildConfig.APPLICATION_ID}.local-media", token).toString(), record.durationMs != null && record.durationMs > 0))
        }
    }

    /** Re-selects a compatible document while preserving the opaque library identity. */
    @ReactMethod
    fun repairLocalAudio(recordId: String, requestId: String, promise: Promise) {
        if (!RECORD_ID.matches(recordId) || !REQUEST_ID.matches(requestId) || pending != null || inFlight != null) {
            promise.resolve(repairReceipt(requestId, recordId, "rejected")); return
        }
        val activity = app.currentActivity ?: run { promise.resolve(repairReceipt(requestId, recordId, "rejected")); return }
        pending = Pending(requestId, promise, recordId, PendingKind.REPAIR)
        activity.runOnUiThread { activity.startActivityForResult(LocalAudioPolicy.pickerIntent(), REPAIR_REQUEST_CODE) }
    }

    /** Removes only the Listen2 relationship after the Room transaction; the source is untouched. */
    @ReactMethod
    fun removeLocalAudio(recordId: String, requestId: String, promise: Promise) {
        if (!RECORD_ID.matches(recordId) || !REQUEST_ID.matches(requestId) || pending != null || inFlight != null) {
            promise.resolve(repairReceipt(requestId, recordId, "rejected")); return
        }
        inFlight = Pending(requestId, promise, recordId)
        val current = inFlight ?: return
        worker.execute {
            try {
                val repository = LibraryRepository.open(app)
                if (!repository.removeLocalRecord(recordId)) {
                    current.promise.resolve(repairReceipt(requestId, recordId, "rejected")); return@execute
                }
                // The database commit comes first. This never calls a document delete API.
                val previous = privateStore.forgetDocument(recordId)
                privateStore.forgetLyric(recordId)
                LocalPlaybackTokens.clearRecord(recordId)
                previous?.let { uri -> runCatching { app.contentResolver.releasePersistableUriPermission(uri, Intent.FLAG_GRANT_READ_URI_PERMISSION) } }
                current.promise.resolve(repairReceipt(requestId, recordId, "repaired"))
    ...
    override fun onActivityResult(activity: Activity, requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode != REQUEST_CODE && requestCode != LRC_REQUEST_CODE && requestCode != REPAIR_REQUEST_CODE) return
        pending = null
        if (resultCode != Activity.RESULT_OK || data == null) {
            if (current.kind == PendingKind.REPAIR) current.promise.resolve(repairReceipt(current.requestId, current.recordId ?: "", "cancelled"))
            else current.promise.resolve(receipt(current.requestId, "cancelled"))
            return
        }
        ...
        if (requestCode == REPAIR_REQUEST_CODE) {
            val recordId = current.recordId ?: run { current.promise.resolve(repairReceipt(current.requestId, "", "rejected")); return }
            val uri = data.data ?: run { current.promise.resolve(repairReceipt(current.requestId, recordId, "rejected")); return }
            inFlight = current; worker.execute { try { repairDocument(current, recordId, uri, data.flags) } finally { if (inFlight === current) inFlight = null } }
            return
        }
        val uris = buildList {
            data.data?.let(::add)
    ...
    private fun repairDocument(current: Pending, recordId: String, uri: android.net.Uri, resultFlags: Int) {
        ...
        val metadata = readMetadata(uri)
        val replacement = SafeLocalRecord(recordId, metadata.title, metadata.artist, "available", metadata.album, metadata.durationMs, metadata.hasArtwork)
        val repository = LibraryRepository.open(app)
    ...
    private fun releaseReadGrant(uri: android.net.Uri) {
    ...
    private fun playback(recordId: String, requestId: String, status: String, uri: String?, seekable: Boolean) = Arguments.createMap().apply {
        putString("recordId", recordId); putString("playbackRequestId", requestId); putString("status", status); putString("privatePlaybackUri", uri); putBoolean("seekable", seekable)
    }
    private fun repairReceipt(requestId: String, recordId: String, status: String) = Arguments.createMap().apply {
        putString("requestId", requestId); putString("recordId", recordId); putString("status", status)
    }
