...
type Stored = { version: number; revision: number; records: LyricSelectionRecord[] };
...
function validKey(value: unknown): value is LyricSelectionKey {
  ...
  if (typeof key.source !== 'string' || typeof key.trackId !== 'string') return false;
  const exact = key.source === 'bilibili' ? parseExactBilibiliTrackId(key.trackId) : null;
  if (key.source === 'bilibili') return Boolean(exact && key.partId === exact.cid);
  return key.partId === null && /^[a-z]+track_[A-Za-z0-9_-]{1,128}$/.test(key.trackId);
}
...
async function read(): Promise<State> {
  ...
  const [zero, one] = await Promise.all([storage().getItem(SLOT(0)), storage().getItem(SLOT(1))]);
  const candidates = [
    { slot: 0 as const, store: parse(zero) },
    { slot: 1 as const, store: parse(one) },
  ].filter((value): value is State => value.store !== null);
  if (!candidates.length) return { slot: 0, store: { version: VERSION, revision: 0, records: [] } };
  candidates.sort((left, right) => right.store.revision - left.store.revision || (head === String(left.slot) ? -1 : 1));
  const selected = candidates[0];
  if (head !== String(selected.slot)) await storage().setItem(HEAD, String(selected.slot));
  return selected;
}
...
export function selectionFromBilibiliCache(
  ...
  if (!record || !validKey({ source: 'bilibili', trackId: record.lyric.trackId, partId: parseExactBilibiliTrackId(record.lyric.trackId)?.cid ?? null })) return null;
  ...
  return {
    ...
    ...(provenance?.mode === 'manual'
      ? { manual: { provider: provenance.matchedProvider, candidateId: provenance.matchedCandidateId } }
      : {}),
  };
}
...
export const lyricSelectionStore = {
  get(key: LyricSelectionKey) {
    ...
    return serial(async () => {
      ...
      return store.records.find(record => identity(record.key) === identity(key)) ?? null;
    });
  },
  put(
    ...
    if (!validKey(input.key) || !Number.isFinite(input.offsetMs) || Math.abs(input.offsetMs) > MAX_LYRIC_OFFSET_MS || (input.manual && input.key.source !== 'bilibili'))
      return Promise.resolve({ status: 'invalid' as const });
    return serial(async () => {
      const { store, slot } = await read();
      const existing = store.records.find(record => identity(record.key) === identity(input.key));
      ...
      await publish({ version: VERSION, revision: store.revision + 1, records: [record, ...store.records.filter(value => identity(value.key) !== identity(input.key))].slice(0, MAX_RECORDS) }, slot);
      return { status: 'ok' as const, record };
    });
  },
  async clearManual(key: LyricSelectionKey, expectedRevision?: number) {
    const current = await this.get(key);
      if (!current) return { status: 'invalid' as const };
    return this.put(
      { key, offsetMs: current.offsetMs },
