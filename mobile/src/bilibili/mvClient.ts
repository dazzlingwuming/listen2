...
const native = NativeModules.Listen2Bilibili as Record<string, unknown> | undefined;
const qualities = new Set<BilibiliMvQualityId>([
  'auto', '16', '32', '64', '74', '80', '112', '116', '120', '125', '126', '127',
]);
const codecs = new Set(['avc1', 'hev1', 'hvc1', 'av01']);
const states = new Set<BilibiliMvPublicState['state']>([
  'idle', 'resolving', 'ready', 'playing', 'paused', 'refreshing', 'error', 'closed',
]);
...
const text = (value: unknown, limit: number): string =>
  typeof value === 'string' && value.trim() && value.length <= limit && !/[\u0000-\u001f]/.test(value)
    ? value.trim()
    : fail();
const bvid = (value: unknown) =>
  /^BV[0-9A-Za-z]{6,32}$/.test(text(value, 40)) ? text(value, 40) : fail('INVALID_REQUEST');
const cid = (value: unknown) =>
  typeof value === 'string' && /^[1-9][0-9]{0,17}$/.test(value) && Number.isSafeInteger(Number(value))
    ? value
    : fail('INVALID_REQUEST');
const handle = (value: unknown) =>
  typeof value === 'string' && /^[A-Za-z0-9_-]{16,96}$/.test(value) ? value : fail('INVALID_RESPONSE');
...
function call(method: string, arg: Record<string, unknown>): Promise<unknown> {
  ...
  if (typeof fn !== 'function') return Promise.reject(new BilibiliMvClientError('UNAVAILABLE'));
  return (fn as (request: Record<string, unknown>) => Promise<unknown>)(arg);
}
function nativeError(raw: Record<string, unknown>): void {
  if (Object.keys(raw).length === 1 && typeof raw.errorCode === 'string' && /^[A-Z_]{3,64}$/.test(raw.errorCode))
    fail(raw.errorCode);
}
function variant(value: unknown): BilibiliMvVariant {
  ...
  return { id: id as BilibiliMvVariant['id'], label: text(raw.label, 80), codec: codec as BilibiliMvVariant['codec'], width: number(raw.width, 7680), height: number(raw.height, 7680) };
}
function state(value: unknown): BilibiliMvPublicState {
  const raw = object(value);
  nativeError(raw);
  exactKeys(raw, ['state', 'handle', 'bvid', 'cid', 'qualityId', 'variants', 'positionMs', 'playIntent', 'refreshing', 'errorCode']);
  ...
  const safeVariants = Array.isArray(variants) && variants.length <= 4 ? variants : fail();
  ...
  const safeErrorCode = errorCode === undefined
    ? undefined
...
function request(value: BilibiliMvRequest): Record<string, unknown> {
  ...
  if (preferredCodecs.length > 4 || new Set(preferredCodecs).size !== preferredCodecs.length || preferredCodecs.some(codec => !codecs.has(codec))) fail('INVALID_REQUEST');
  return {
...
export const bilibiliMvClient = {
  ...
  selectQuality: (opaqueHandle: string, qualityId: BilibiliMvQualityId) =>
    call('mvSelectQuality', { handle: handle(opaqueHandle), qualityId: quality(qualityId) }).then(state),
  sync: (opaqueHandle: string, positionMs: number, playIntent: boolean) =>
    call('mvSync', { handle: handle(opaqueHandle), positionMs: number(positionMs, 24 * 60 * 60 * 1000), playIntent: Boolean(playIntent) }).then(state),
  refresh: (opaqueHandle: string) => call('mvRefresh', { handle: handle(opaqueHandle) }).then(state),
  ...
  syncActive: (positionMs: number, playIntent: boolean) =>
    activeHandle
      ? call('mvSync', { handle: activeHandle, positionMs: number(positionMs, 24 * 60 * 60 * 1000), playIntent: Boolean(playIntent) }).then(state)
      : Promise.resolve(null),
  enterFullscreen: (opaqueHandle: string) =>
    call('mvEnterFullscreen', { handle: handle(opaqueHandle) }).then(value => booleanReply(value)),
  exitFullscreen: (opaqueHandle: string) =>
    call('mvExitFullscreen', { handle: handle(opaqueHandle) }).then(value => booleanReply(value)),
  requestPip: (opaqueHandle: string) =>
    call('mvRequestPip', { handle: handle(opaqueHandle) }).then(value => booleanReply(value)),
};
