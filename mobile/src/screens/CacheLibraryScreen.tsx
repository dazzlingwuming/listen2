...
export function CacheLibraryScreen() {
  ...
  const act = (action: 'cancel'|'retry'|'repair'|'remove'|'clearEligible'|'promote', id?: string) => void (action === 'promote' ? offlineAudio.promote('', id || '') : offlineAudio.action(action as any, id)).then(setSnapshot);
  return <ScreenLayout title="缓存与下载" subtitle={`已用 ${Math.floor(snapshot.usedBytes / 1024 / 1024)} MiB；预留 ${Math.floor(snapshot.reservedBytes / 1024 / 1024)} MiB`}>
    ...
    {entries.map(entry => <View key={entry.operationId} style={sectionStyles.card}><Text style={text.body}>{entry.title}</Text><Text style={text.meta}>{entry.artist} · {entry.owners.join('、') || '无 owner'} · {entry.status}</Text>{entry.errorCode ? <Text accessibilityRole="alert" style={text.meta}>{offlineDownloadErrorCopy(entry.errorCode)}</Text> : null}<View style={{ flexDirection: 'row', gap: 12 }}><Pressable accessibilityLabel={`永久下载${entry.title}`} onPress={() => act('promote', entry.trackId)}><Text style={text.meta}>永久下载</Text></Pressable><Pressable accessibilityLabel={`重试或修复${entry.title}`} onPress={() => act(entry.status === 'ready' ? 'repair' : 'retry', entry.operationId)}><Text style={text.meta}>重试/修复</Text></Pressable><Pressable accessibilityLabel={`删除${entry.title}`} onPress={() => Alert.alert('删除此缓存？', '共享或明确下载内容仅在对应 owner 被移除后才删除。', [{ text: '取消', style: 'cancel' }, { text: '删除', style: 'destructive', onPress: () => act('remove', entry.operationId) }])}><Text style={text.meta}>删除</Text></Pressable></View></View>)}
  </ScreenLayout>;
}
