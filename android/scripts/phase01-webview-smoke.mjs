...
const stateExpression = `(() => { const root=document.querySelector('.bilibili-mobile-search'); const scope=root && window.angular ? angular.element(root).scope() : null; const search=scope?.bilibiliSearch || {}; const detail=scope?.bilibiliDetail || {}; const text=document.body.innerText; const progress=(document.querySelector('.current')?.innerText || '0:00').trim(); const targetRects=[...document.querySelectorAll('button')].filter(b => b.getAttribute('aria-label')==='提交哔哩哔哩搜索' || b.textContent.includes('播放此分P')).map(b => { const r=b.getBoundingClientRect(); return {w:Math.round(r.width),h:Math.round(r.height)}; }); return {shell:Boolean(root && document.querySelector('.mobile-tabbar')),searchState:String(search.state||''),resultCount:document.querySelectorAll('.bilibili-mobile-result').length,detailState:String(detail.state||''),partCount:document.querySelectorAll('.bilibili-mobile-parts button').length,bvid:String(detail.bvid||''),cid:String(detail.selectedCid||''),progress:/^[0-9]+:[0-9]{2}$/.test(progress)?progress:'0:00',playback:text.includes('正在播放')?'playing':text.includes('已暂停')?'paused':text.includes('正在准备播放')?'resolving':'',lyric:text.includes('暂无可用歌词')?'unavailable':text.includes('歌词暂时无法加载')?'error':text.includes('正在获取歌词…')?'loading':'',width:Math.round(innerWidth),height:Math.round(innerHeight),overflow:document.documentElement.scrollWidth>innerWidth+1,targets:targetRects}; })()`;
