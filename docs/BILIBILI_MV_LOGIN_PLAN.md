...
已完成：主进程 B 站适配层、二维码扫码状态机、Electron 默认会话 Cookie 持久化、`safeStorage` 加密保存刷新令牌、限频 Cookie 检查/刷新/确认、WBI DASH 清单、实际可播放音质选择、静音 MV 与 Howler 时钟同步、画质选择、全屏、退出清理及单元测试。
...
- 打开后视频替换或覆盖现有封面/可视化区域；
- 歌词面板保持可见，用户仍可切换原文/译文；
- 视频上提供：
  - 关闭 MV；
  - 画质；
  - 全屏；
