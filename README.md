...
Listen2 做的是整理和改善这些可访问内容的播放体验，而不是解锁用户原本无权访问的资源。当前版本集中完成了六件事：
...
6. 在桌面端自动保存已经播放的 Bilibili 音频与歌词，让断网时仍有内容可听。
...
- 桌面端会在歌曲真正开始播放后，低优先级后台缓存完整的 Bilibili 音频；第一次播放仍直接使用在线流，不等待下载。
- 只有完整下载并通过长度校验的文件才会进入离线缓存，临时文件、失败下载和不完整内容不会交给播放器。
- 默认启用并提供 2 GB 容量，也可在设置中切换为 1 GB、5 GB、10 GB 或不限制；达到上限时仅按最近最少使用顺序清理音频文件。
- 缓存命中时可在没有网络的情况下播放；本地文件损坏时会删除该条缓存，并在网络可用时回到原有 CDN 恢复流程。
- 设置页可以查看已用空间、缓存曲目数和后台任务，也可以清空全部音频缓存，或删除当前歌曲的音频、歌词、翻译与偏移数据。
- Chrome 扩展不启用桌面音频缓存；这部分数据也不会进入歌单备份或 GitHub Gist。
...
- DeepSeek 成功译文随对应歌词持久化，不再按固定歌曲数量静默淘汰；只有译文与当前原文完全对应时才会显示。
...
- 新备份文件只包含我的歌单和收藏歌单，不会导出登录凭据、本地音乐路径、主题或歌词设置。
- 本地文件和 GitHub Gist 均默认使用合并导入：保留当前设备的歌单及顺序，再追加备份中的歌单。
- 同名歌单作为独立歌单保留；完全相同的歌单会跳过，ID 冲突但内容不同时会生成新 ID，不覆盖本机版本。
- 兼容旧版 `listen1_backup.json`，但导入时只提取歌单数据，不再将整个浏览器存储覆盖到新设备。
- 仍保留“覆盖当前歌单”高级选项，但必须通过应用内二次确认，且只替换歌单集合。
...
- 桌面端 Bilibili 音频离线缓存、自动歌词持久化和逐曲本地数据清理已经接入。
- 本地文件与 GitHub Gist 歌单备份已改为默认合并导入，并兼容旧版备份。
...
| Bilibili  | 基础搜索与播放             | 更新接口、清理标题、补充元数据、多 CDN、有界恢复与桌面离线缓存 |
| 账号与 MV | 无                         | 本地扫码、权限内音质、分 P MV、画质选择与全屏                  |
| 动态效果  | 基础播放反馈               | 真实音频驱动的频谱和环形视觉                                   |
| 歌词      | 依赖来源直接返回           | 自动匹配与持久化、跨源译文、整首机器翻译、双语时间轴           |
