...
- 首次播放、尚未分析、采样率不受支持或无法安全分析的歌曲保持原音量，不会在播放过程中突然追踪调节。
- 已分析歌曲只在播放时应用固定增益，保留歌曲内部的动态关系；用户音量、静音和系统音量仍然独立。
- 响度结果与缓存文件内容哈希绑定，音频被淘汰、清空或删除当前歌曲本地数据时会一并移除。
...
- 当前响度分析器仅处理确认是 48 kHz、时长不超过 15 分钟且符合资源限制的完整 Bilibili 缓存；其他内容保持原音量。
